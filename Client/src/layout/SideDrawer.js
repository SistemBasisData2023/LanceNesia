...
const SideDrawer = (props) => {
  const drawerContent = (
    <>
      ...
      <ListStyle>
        {links.map((el) => (
          <CustomListItem key={el.id} path={el.path} icon={el.icon} title={el.title} onClick={props.onClose} />
        ))}
      </ListStyle>
      ...
      <GetMoreStyle>
        ...
        <Typography variant="body2" component="h5">
          Go To Website To Get More Help
        </Typography>

        <a href="https://abdfikih.site/" target="_blank" rel="noreferrer">
          Got To Website
        </a>
      </GetMoreStyle>
    </>
